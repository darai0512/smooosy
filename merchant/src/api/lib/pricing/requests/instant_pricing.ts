...
function createRequestPriceInstant(request) {
  const price = new Price()

  price.addAdjustment(PriceComponents.priceBasedCostAdjustment({
    priceValue: request.priceValueResult.total,
    pricesToPoints: request.pricesToPoints,
  }))
  ...
  price.addAdjustment(PriceComponents.baseValueBasedCapAdjustment({
    ...
    minMultiplier: 0.7,
    maxMultiplier: 1.35,
    baseAdjustmentName: 'PRICE_BASED_COST + DESCRIPTION_BOOST',
  }))
...
function totalAdjustment() {
  return new Adjustment({
    ...
    formula:
      `min(
        (
          PRICE_BASED_COST + DESCRIPTION_BOOST) * (
            1 + min(
              PHONE_BOOST + CORPORATE_BOOST + MESSAGE_LENGTH_BOOST + POINT_MULTIPLIER_BOOST,
              BASE_VALUE_BASED_CAP
            )
          ) *
...
module.exports = {createRequestPriceInstant}